...
const Nexmo = require('nexmo'); // for sms sending and receiving
...
//   apiKey:'3dbad042' ,
//   apiSecret:'Z3iOwLs1H80ScC2O'
// })

// const from = 'Nexmo';
// const to = '918273825126';
// const text = 'Hello from Nexmo';

// nexmo.message.sendSms(from, to, text,(err,responseData)=>{
//   if(err)
//   {
//     console.log("err in sending sms"+err)
//   }
//   else
//   {
...
//inserting data
  // let ins = new adminLogin({'username':'manish','password':sha1('admin')})
...
app.listen(7788,function(){
	console.log("eshop listening to port : 7788");
})
...
app.get('/api/fetch_latest_products',function(req,res){
  let query=proModel.find().sort({'date_created':-1});
...
app.post('/api/save_order',function(req,res)
{
   upload(req,res,function(err)
   {
       ...
       else
       {
           ...
           let ins=new order({'buyer':buyer,'oData':odata,'orderId':orderid,'email':email,'delivery_status':delivery_status});
...
// fetching order in AdminPanel

app.get('/api/get_orders',function(req,res) {
  
  order.find({},function(err,data){
    if(err){
      res.json({'msg':'error occured in fetching order data for adminpanel'})
    }
      else if(data.length==0)
      {
        res.json({'err':1,'msg':'order data not found'})
...
// marking products as delivered

app.post('/api/mark_as_delivered',function(req,res){
     ...
     upload(req,res,function(err)
   {
       ...
       else
       {
         let pid = req.body.id;
          ...
          order.update({ "oData[0]":pid},{$set:{'delivery_status':true}},function(err){
            if(err){
              console.log(err);
              res.json({'err':1,'msg':'Error in marking delivered'});
            }
              else{

                res.json({'err':0,'msg':'Product marked as delivered'});
              }
          }) 
...
//fetching apple mobiles only to show in header mobile options


app.get('/api/fetch_apple_mob',function(req,res){
  let query=proModel.find({'brand':'APPLE','productCategory':'Mobile Devices'}).sort({'date_created':-1}).limit(5);

    query.exec(function(err,data){
    if(err){
      console.log("err in fetchig apple mobiles");
      res.json({'err':1,'msg':'err in fetchig apple product data'})
    }
    else{
      console.log("latest apple mobiles fetch successfully");
      res.json({'err':0,'appleMob':data})
    }
  })
  })
  

  //fetching samsung mobiles only to show in header mobile options

app.get('/api/fetch_samsung_mob',function(req,res){
  let query=proModel.find({'brand':'SAMSUNG','productCategory':'Mobile Devices'}).sort({'date_created':-1}).limit(5);

    query.exec(function(err,data){
    if(err){
      console.log("err in fetchig samsung mobiles");
      res.json({'err':1,'msg':'err in fetchig samsung product data'})
    }
    else{
      console.log("latest samsung mobiles fetch successfully");
      res.json({'err':0,'samMob':data})
    }
  })
  })

  //fetching mi mobiles only to show in header mobile options

app.get('/api/fetch_mi_mob',function(req,res){
  let query=proModel.find({'brand':'MI','productCategory':'Mobile Devices'}).sort({'date_created':-1}).limit(5);

    query.exec(function(err,data){
    if(err){
      console.log("err in fetchig mi mobiles");
      res.json({'err':1,'msg':'err in fetchig mi product data'})
    }
    else{
      console.log("latest mi mobiles fetch successfully");
      res.json({'err':0,'miMob':data})
    }
  })
  })


  //fetching laptops in below 4 cases  to show in header mobile options

app.get('/api/fetch_dell_lap',function(req,res){
  let query=proModel.find({'brand':'DELL','productCategory':'Laptops'}).sort({'date_created':-1}).limit(5);

    query.exec(function(err,data){
    if(err){
      console.log("err in fetchig dell laptops");
      res.json({'err':1,'msg':'err in fetchig dell lap product data'})
    }
    else{
      console.log("latest dell lap fetch successfully");
      res.json({'err':0,'dellLap':data})
    }
  })
  })

app.get('/api/fetch_hp_lap',function(req,res){
  let query=proModel.find({'brand':'HP','productCategory':'Laptops'}).sort({'date_created':-1}).limit(5);

    query.exec(function(err,data){
    if(err){
      console.log("err in fetchig hphp laptops");
      res.json({'err':1,'msg':'err in fetchig hp lap product data'})
    }
    else{
      console.log("latest hp lap fetch successfully");
      res.json({'err':0,'hpLap':data})
    }
  })
  })

app.get('/api/fetch_asus_lap',function(req,res){
  let query=proModel.find({'brand':'ASUS','productCategory':'Laptops'}).sort({'date_created':-1}).limit(5);

    query.exec(function(err,data){
    if(err){
      console.log("err in fetchig asus laptops");
      res.json({'err':1,'msg':'err in fetchig asus lap product data'})
    }
    else{
      console.log("latest asus lap fetch successfully");
      res.json({'err':0,'asusLap':data})
    }
  })
  })

app.get('/api/fetch_alien_lap',function(req,res){
  let query=proModel.find({'brand':'ALIENWARE','productCategory':'Laptops'}).sort({'date_created':-1}).limit(5);

    query.exec(function(err,data){
    if(err){
      console.log("err in fetchig ALIENWARE laptops");
      res.json({'err':1,'msg':'err in fetchig ALIENWARE lap product data'})
    }
    else{
      console.log("latest asus ALIENWARE fetch successfully");
      res.json({'err':0,'alienLap':data})
    }
  })
  })
  
//app.get('*',(req,res)=>{
	//return res.sendFile(paths.join(__dirname,'../../MyProject/AdminPanel/dist/AdminPanel/index.html'))
//})
